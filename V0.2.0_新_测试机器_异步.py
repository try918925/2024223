...
class ImageProcessWorker(threading.Thread):
    ...
    def process_frames(self, frames, id, num, status):
        ...
        if state:
            ...
            frame_name = "left" if id == 0 else "right" if id == 2 else "top"
            cv2.imwrite(f'tmp/{frame_name}.jpg', resized_image)
            print(frame_name, ":写入成功")
...
class ImageProcessWorker2(threading.Thread):
    ...
    def clear_writer_img(self, camera_id, frame):
        frame_name = "front" if camera_id == 0 else "rear"
        file_path = f'tmp/{frame_name}.jpg'
        if camera_id == 0:
...
class SaveProcessWorker(threading.Thread):
    ...
    def check(self):
        ...
        self.check_and_save(current_time, output_folder)

    def check_and_save(self, output_folder):
        position = ["left", "top", "right", "front", "rear"]
        li = []
        for tmp_data in position:
            file_path = f'tmp/{tmp_data}.jpg'
            output_path = os.path.join(output_folder, f"{tmp_data}.jpg")
    ...
    def send_zmq_string(self, li):
        ...
        data = f"[C|{timestamp}|{self.ch}|{li[3]}|{li[4]}|{li[0]}|{li[2]}|{li[1]}|{self.ip_address}|D]"
        # 发布数据
    ...
    def check_files_existence(self, folder_path):
        ...
        # files_to_check = ['top.jpg', 'left.jpg', 'right.jpg', "6666.jpg"]
        # 检查每个文件是否存在
