...
class ImageCaptureProcess(Process):
    ...
    def run(self):
        ...
        try:
            ...
            while True:
                ...
                # print(f"{self.direction}传输一张图片的时间为：",time.time()-start_time)

                count += 1

                now_time = time.time()
                get_time = now_time - start_time
                if get_time >= 1:
                    # print(f"{get_time}的时间内得到了:{count}")
                    count = 0
                    start_time = time.time()
    ...
    def put_rear_front(self, frame):
        ...
        frame_crop = frame.colRange(self.roi_cols[0], self.roi_cols[1]).rowRange(self.roi_rows[0],
                                                                                 self.roi_rows[1])
        frame_cpu = frame_crop.download()  # 裁剪后的图像下载到CPU
    ...
    def put_left_right_top(self, frame):
        ...
        frame_crop = frame.colRange(self.roi_cols[0], self.roi_cols[1]).rowRange(self.roi_rows[0],
                                                                                 self.roi_rows[1])
        frame_cpu_crop = frame_crop.download()
        frame_cpu_orgin = frame.download()  # 全图下载到CPU
        frame_cpu_crop = frame_cpu_crop[:, :, :3]
        frame_cpu_orgin = frame_cpu_orgin[:, :, :3]
        frame_cpu_orgin = self.crop_images(frame_cpu_orgin, self.direction)
        self.queue.put((self.camera_id, frame_cpu_orgin, frame_cpu_crop))
...
class ImageProcessWorker(QThread):
    ...
    def process_frames(self, frames, id):
        ...
        my_stiching = ts.stiching_no_crop()
        stitching_method = {1: "top", 0: "left", 2: "right"}.get(id, "left")
        result = my_stiching.stiching(frames, stitching_method)
