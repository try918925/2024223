...
class ImageCaptureProcess(Process):
        ...
        try:
            ...
            while True:
                ...
                if self.direction in self.front_rear:
                    target_interval = 1 / self.frame_rate["front_rear"]
                    if elapsed_time < target_interval:
                        time.sleep(target_interval - elapsed_time)
                    self.front_rear_frame(frame)
                    self.last_time = time.time()
                elif self.direction in self.left_right:
                    target_interval = 1 / self.frame_rate["left_right"]
                    if elapsed_time < target_interval:
                        time.sleep(target_interval - elapsed_time)
                    self.left_right_frame(frame)
                    self.last_time = time.time()
                else:
                    target_interval = 1 / self.frame_rate["top"]
                    if elapsed_time < target_interval:
                        time.sleep(target_interval - elapsed_time)
    ...
    def front_rear_frame(self, frame):
        frame_crop = frame.colRange(self.roi_cols[0], self.roi_cols[1]).rowRange(self.roi_rows[0],
                                                                                 self.roi_rows[1])
        frame_cpu = frame_crop.download()
        frame_cpu = frame_cpu[:, :, :3]
        self.queue.put((self.camera_id, frame_cpu))

    def left_right_frame(self, frame):
        frame_resized = cv2.cuda.resize(frame, self.dsize)
        frame_crop = frame.colRange(self.roi_cols[0], self.roi_cols[1]).rowRange(self.roi_rows[0],
                                                                                 self.roi_rows[1])
        frame_cpu_crop = frame_crop.download()
        frame_cpu_resized = frame_resized.download()
    ...
    def run(self):
        ...
        while run_Flag:
            if not self.queue.empty():
                ...
                # print("ImageProcessWorker2", self.camera_id, self.queue.qsize(), "\n")
