...
def writer_img(file_path, frame):
...
class ImageProcessWorker(QThread):
    ...
    def process_frames(self, frames, id):
        ...
        process = Process(target=writer_img, args=(f'tmp/{id}.jpg', result))
        process.start()
...
class ImageProcessWorker2(QThread):
            ...
            try:
                if not self.queue.empty():
                    ...
                    if self.my_container_detect.non_truck > 12 and self.my_container_detect.new_truck:
                        ...
                        self.executor.submit(self.clear_writer_img, self.camera_id, f'tmp/{self.camera_id + 3}.jpg', reuslt_dict["img"])
    ...
    def clear_writer_img(self, camera_id, file_path, frame):
        if camera_id == 0:
            self.delete_old_files()
        cv2.imwrite(file_path, frame)


def write_shuangxiang(ocr_path, xiangti):
...
class SaveProcessWorker(QThread):
    ...
    def check_and_save(self, current_date, current_time, output_folder):
        ...
        try:
            ...
            # with open(ocr_path, 'a', encoding='utf-8') as file:
            #     file.write(xiangti + '\n')
            process = Process(target=write_shuangxiang, args=(ocr_path, xiangti))
            process.start()
            print("----执行完成----")
            xiangti = ''
        except Exception as e:
...
def write_ocr_data(ultra_result, ocr_data_dict):
    ...
    def run(self):
        ...
        while run_Flag:
            ...
            if id == 10:
                ...
                self.executor.submit(self.ocrMessage_emit, 1, "箱门朝前")
            if id == 11:
                ocr_data_dict["rear"] = "箱门朝后"
                self.executor.submit(self.ocrMessage_emit, 1, "箱门朝后")
            if id != 5:
                if id == 0:
            ...
            if id == 5:
                ...
                process = Process(target=write_ocr_data, args=(ultra_result_one, copy.deepcopy(ocr_data_dict)))
                process.start()

                self.executor.submit(self.ocrMessage_emit, 1, ultra_result_one)
                ocr_data_dict = {"front": "", "rear": ""}
...
def writer_rec_data(ultra_result):
        ...
        while run_Flag:
            ...
            if id == 5:
                ...
                if len(save_frame) > 0:
                    ...
                    process = Process(target=writer_img, args=('tmp/chepai.jpg', final_save))
                    process.start()
                ...
                process = Process(target=writer_rec_data, args=(ultra_result,))
                process.start()
