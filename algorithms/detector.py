...
from models_sx.common import DetectMultiBackend
from utils_sx.general import (LOGGER, Profile, check_file, check_img_size, colorstr, cv2,
                           increment_path, non_max_suppression, strip_optimizer, xyxy2xywh)
from utils_sx.torch_utils import select_device
from models_sx.yolo import Model
# --------------------------------------------------
...
class YOLOv5Detector(object):
    ...
    def __init__(self, target_size, padding_color, conf_thres, iou_thres, device, net_conf, weight_path, classes):
        ...
        self._model = DetectMultiBackend(weight_path, device=device, dnn=False, data=None, fp16=True)
    ...
    def det(self, image_list: list, *args, **kwargs):
        ...
        batch_pred = non_max_suppression(
            ...
            classes=[class_id for class_id in range(len(self._classes))],  # notice to be int
            # classes=self._classes,  # notice to be int
            agnostic=False
        )
            # ----------------------------------------
    ...
    def _preprocess(self, image_list: list) -> tuple:
        ...
        for image in image_list:
            ...
            image_tensor = image_tensor.half() # 精度减半 image_tensor.half() / image_tensor.float()
                                               # 要与 export.py 对应 --half 参数对应
            image_tensor /= 255.0
        ...
        for idx, (pred_item, image_size) in enumerate(zip(batch_pred, batch_image_size)):
            ...
            if (pred_item is not None) and len(pred_item):
                ...
                for *p1p2, conf, klass_idx in pred_item:
                    ...
                    result_item.append((klass_id, klass_name, score, (x1, y1, x2, y2), (xo, yo, w, h)))
