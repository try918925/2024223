        ...
        try:
            ...
            while True:
                ...
                if self.direction in self.front_rear:
                    ...
                    # self.direction in self.left_right_top:
...
class ImageProcessWorker(QThread):
    ...
    def process_frames_stiching(self, frames, id):
        print(f"1111111111111111111111:{id}")
    ...
    def process_frames(self, frames, id):
        ...
        stitch_process = Process(target=self.process_frames_stiching, args=(frames, id))
        stitch_process.start()
        stitch_process.join()

        result = cv2.imread(f"{id}.jpg")
        if id == 2:
