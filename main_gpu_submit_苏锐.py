    ...
    def run(self):
        ...
        while run_Flag:
            if not self.result_queue.empty():
                ...
                print("self.result_queue.get():",self.result_queue.qsize())
                if result == 1:
                    consecutive_zero_count = 0
                ...
                # print(f"{self.direction}相机:处理照片结果时间:", time.time() - start_time, "/n")
...
def write_ocr_data(ocr_path, xiangti):
    ...
    with open(ocr_path, 'w', encoding='utf-8') as file:
        if ultra_result_one:
            file.write(ultra_result_one + '/n')
        if ultra_result_two:
            file.write(ultra_result_two + '/n')
        file.write(f"{data_data['front']}/n{data_data['rear']}/n{xiangti}")
...
def returns_two_similar_data(data, threshold=0.6):
    ...
    for item in data:
        ...
        for similar_item in similar_data[1:]:
            if similarity_ratio(item[0], similar_item[0]) >= threshold:
        ...
        while run_Flag:
            ...
            if id != 5:
                if id == 0:
                    ...
                    # path = f"E:/luxiang/frames/{time_str()}.jpg"
                    # self.executor.submit(self.write_frame, path, frame)
                ...
                #         count += 1
                #         if count >= 10:
                #             ultra_result.append(det_ocr.vote2res(result_list))
                #             result_list.clear()
                #     else:
                #         count = 0
