...
def stichAll_new(imgLst, offLst, v):
    if not imgLst or not offLst:
        return None

    temp = imgLst[0]  # 初始化 temp 为第一个图像
    rectify_off = rectifyOff(offLst)  # 提前计算校正后的 offset

    for i in range(1, len(imgLst)):  # 从第二张图开始处理
        img2 = imgLst[i]
        off = offLst[i - 1]

        # 优化条件判断，将 off 的不同情况进行处理
        if off > 1:
...
def estimate_translation_orb(img1, img2, v):
    ...
    if v == 'right':
        ...
        # else:
        #     return 0
        orb = cv2.ORB_create(nfeatures=1500, scaleFactor=1.3001, nlevels=7, edgeThreshold=21,
                             firstLevel=0, WTA_K=2, scoreType=cv2.ORB_HARRIS_SCORE, patchSize=31, fastThreshold=15)
        keypoints1, descriptors1 = orb.detectAndCompute(img1, None)
        keypoints2, descriptors2 = orb.detectAndCompute(img2, None)
