...
class ImageCaptureProcess(Process):
    ...
    def put_rear_front(self, frame):
        ...
        smaller_gpu_img = cv2.cuda.resize(frame, (width // 2, height // 2))
        # 再放大回到原来的尺寸
        restored_gpu_img = cv2.cuda.resize(smaller_gpu_img, (width, height))
        # 将处理后的图像从 GPU 下载回到 CPU 内存
        result_img = restored_gpu_img.download()
        result_img = result_img[:, :, :3]  # 裁剪通道
        self.queue.put((self.camera_id, result_img))
