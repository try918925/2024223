...
class ImageProcessWorker(QThread):
    ...
    def process_frames(self, frames, id):
        ...
        self.executor.submit(self.writer_img, file_path, result)
            ...
            try:
                if not self.queue.empty():
                    ...
                    if self.my_container_detect.non_truck > 10 and self.my_container_detect.new_truck:
                        # 当连续14帧(约1s)没有集装箱面，且之前有卡车进入时，获取前一段时间面积最大帧
                        ...
                        self.executor.submit(self.writer_img, file_path, reuslt_dict["img"])
