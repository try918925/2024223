            ...
            if not self.queue.empty():
                ...
                if self.camera_id == 0:
                    if self.frame_counter == 4:
                        ...
                        self.frame_counter = 0
                self.my_container_detect.process_frame(frame[400:1400, 400:1800])
                # cv2.imwrite(f'./doorTestImg/{time.time()}.jpg', frame)
                if self.my_container_detect.non_truck > 20 and self.my_container_detect.new_truck:
        ...
        while True:
            ...
            if id !=5:
                ...
                if id == 6:
                    h, w = frame.shape[:2]
                    n = 300
                    frame_crop = frame[:,h//2-n:]
                    result = self.ocr.process_imgs([frame, frame_crop],"front") 
                if id == 7:
                    h, w = frame.shape[:2]
                    n = 300
                    frame_crop = frame[:,h//2-n:]
                    result = self.ocr.process_imgs([frame, frame_crop],"rear") 
...
class RecThread(QThread): 
    ...
    def run(self):
        ...
        while True:
            id,frame = self.queue.get()
            if id != 5:
                use_frame = frame[500:1400, 600:1700]
