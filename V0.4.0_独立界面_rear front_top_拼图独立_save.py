...
def camera_model_data(filed_name):
    return {
        "params_left": {
            "file_path": f"C:/TianjinGangTest/22fps/{filed_name}/left.mp4",
            "roi": [400, 1300, 50, 950],
            "id": 0,
            "queue": "",
            "direction": "left"
        ...
        "params_right": {
            "file_path": f"C:/TianjinGangTest/22fps/{filed_name}/right.mp4",
        ...
        "params_top": {
            "file_path": f"C:/TianjinGangTest/22fps/{filed_name}/top.mp4",
        ...
        "params_front": {
            "file_path": f"C:/TianjinGangTest/22fps/{filed_name}/front.mp4",
            "roi": [450, 1440, 780, 1700],
            "id": 0,
            "direction": "front"
        },
        "params_rear": {
            "file_path": f"C:/TianjinGangTest/22fps/{filed_name}/rear.mp4",
...
def front_rear_capture_process(camera_info):
    ...
    for camera, original_queues in camera_info:
        if camera["direction"] in ["front", "rear"]:
            process = FrontRearCaptureProcess(camera, original_queues, sleep_time=0.04)
            thread_list.append(process)
            process.start()
        if camera["direction"] in ["top"]:
            process = TopCaptureProcess(camera, original_queues, sleep_time=0.2)
            thread_list.append(process)
            process.start()
...
class MainWindow(QMainWindow):
    def __init__(self,filed_name, log_queue):
        ...
        self.front_rear_capture = Process(
            ...
            args=(zip(self.camera_config.values(), self.original_queues),self.log_queue,)
        )

        self.top_rear_capture = Process(
            target=top_left_worker_process,
            args=(zip(list(self.camera_config.values())[:3], self.result_queues[:3]), self.ocr_queue, self.resource_queue,self.log_queue)
        )

        self.capture_processes = [RightLeftCaptureProcess(camera, original_queues, sleep_time=0.005) for
...
if __name__ == "__main__":
    ...
    folder_filed = os.path.join(r"C:\Users\hello\Desktop\2024223\TianJinGangText", folder_name)
