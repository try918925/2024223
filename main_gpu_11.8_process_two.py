        ...
        try:
            ...
            while True:
                ...
                if not ret or frame is None:
                    decoder = cv2.cudacodec.createVideoReader(self.file_path)
                    logger.error(f"{self.direction}:图片断流")
                    continue
                self.transfer_frame_gpu(frame)
                # logger.info(f"{self.direction}, 传入图片数量时间为:, {time.time() - start_time}")
        except Exception as e:
            logger.error(f"ImageCaptureProcess---{self.direction}:图片读取有问题")
...
class ImageProcessWorker(QThread):
    ...
    def process_frames(self, frames, id):
        logger.info(f"{self.direction}, :拼图的图片数量为:{len(frames)}", )
        ...
        # logger.info(f"{self.direction}:拼图完成 页面展示")
            ...
            if not self.result_queue.empty():
                ...
                if result == 1:
                    ...
                    if self.consecutive_true_count >= self.true_threshold:
                        ...
                        if self.car_in:
                            # consecutive_zero_count = 0
                            # logger.info(f"{self.direction}:车辆驶离")
                            self.car_in = False
                ...
                if result == 0:
                    ...
                    self.count + 1
                    self.frames_to_process.append(original_frame)
                    # ocr队列
        ...
        while run_Flag:
            try:
                if not self.queue.empty():
                    ...
                    # logger.info(f"{self.direction}的识别时间为:{time.time() - start_time}")
                    ...
                    if self.my_container_detect.non_truck > 10 and self.my_container_detect.new_truck:
                        ...
                        # logger.info('当连续18帧(约1s)没有集装箱面，且之前有卡车进入时，获取前一段时间面积最大帧')
                ...
                logger.error(f"ImageProcessWorker2--{self.direction}:error:{error}")
...
class ImageProcessRecognize2(Process):

    def __init__(self, camera_info, original_queue, result_queue, rec_queue, ocr_queue):
        super().__init__()
        self.original_queue = original_queue
        self.result_queue = result_queue
        ...
        while run_Flag:
            try:
                if not self.original_queue.empty():
                    ...
                    self.my_container_detect.process_frame(frame)
                    _, reuslt_dict = self.my_container_detect.get_result()
                    print(f"{self.direction}的识别时间为:{time.time() - start_time}")
                    if len(reuslt_dict) > 0:
                        res_dict_lst.append(reuslt_dict)
    ...
    def run(self):
        ...
        while run_Flag:
            ...
            if id == 5:
                ...
                result_list = []
                # logger.info(f"ultra ocr result:{ultra_result}")
                ...
                with open(file_path, 'w', encoding='utf-8') as file:
                    ...
                    # logger.info(f"箱号识别结果已保存:{ultra_result},{door_in}")
    ...
    def run(self):
        ...
        while run_Flag:
            ...
            if id == 5:
                ...
                save_frame = []
                thres_lst = []
                # logger.info(f"ultra rec result:{ultra_result}")
                # 写入结果到文件中
                file_path = 'tmp/rec.txt'
                with open(file_path, 'w', encoding='utf-8') as file:
                    file.write(ultra_result)
                # logger.info("结果已保存到文件:file_path")
