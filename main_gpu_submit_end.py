...
class SaveProcessWorker(QThread):
    ...
    def check_and_save(self, current_date, current_time, output_folder):
        ...
        try:
            ...
            # self.write_ocr_data(ocr_path, xiangti)
            self.executor.submit(self.write_ocr_data, ocr_path, xiangti)
            xiangti = ''
        except Exception as e:
            # 在这里处理异常
    ...
    def write_ocr_data(self, ocr_path, xiangti):
        ultra_result_one = ""
        ultra_result_two = ""

        with open(ocr_path, 'r', encoding='utf-8') as f:
            data_data = json.load(f)
        print("data_data加载数据:", data_data)

        start_time = time.time()

        if xiangti == "双箱":
            similar_data, dissimilar_data = self.returns_two_similar_data(data_data["ocr_data"])
    ...
    def returns_two_similar_data(self, data, threshold=0.5):
        ...
        for item in data:
            ...
            for similar_item in similar_data:
                if self.similarity_ratio(item[0], similar_item[0]) >= SIMILARITY_THRESHOLD:
...
class UpdataRecOcrThread(QThread):
    ...
    def run(self):
        while True:
            if not self.queue.empty():
                place, id, data = self.queue.get()
                self.executor.submit(self.recMessage_emit, place, id, data)

    def recMessage_emit(self, place, id, data):
        if place == "rec":
...
class OcrRecognizeProcess(Process):
    def __init__(self, ocr_queue, rec_orc_queue):
        super().__init__()
        self.queue = ocr_queue
        self.ocr_data_queue = rec_orc_queue
    ...
    def run(self):
        ...
        self.ocr = self.det_ocr.OCR_process(config_dict)
        ...
        while True:
            ...
            if id == 10:
                ...
                self.ocr_data_queue.put(("ocr", 1, "箱门朝前"))
            if id == 11:
                ocr_data_dict["rear"] = "箱门朝后"
                self.ocr_data_queue.put(("ocr", 1, "箱门朝后"))
            if id != 5:
                if id == 0:
                ...
                if id == 6:
                    frame_ues = self.cv2.GaussianBlur(frame, (5, 5), 0)
            ...
            if id == 5:
                ...
                self.ocr_data_queue.put(("ocr", 1, result_list))

                ocr_data_dict.update({"ocr_data": result_list.copy()})
                print("箱号识别数据:", ocr_data_dict)
                self.executor.submit(self.write_ocr_data, ocr_data_dict.copy())
...
class RecRecognizeProcess(Process):
    def __init__(self, rec_queue, rec_orc_queue):
        super().__init__()
        self.queue = rec_queue
        self.rec_data_queue = rec_orc_queue
    ...
    def run(self):
        ...
        while True:
            id, frame = self.queue.get()
            if id != 5:
                use_frame = self.cv2.GaussianBlur(frame, (5, 5), 0)
            ...
            if id == 5:
                ultra_result, save_frame_idx = self.det_ocr_car.get_finalResult(result_list)
                if len(save_frame) > 0:
                ...
                self.rec_data_queue.put(("rec", 1, ultra_result))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.rec_orc_queue = Queue()

        self.ocr_rec_thread = UpdataRecOcrThread(self.rec_orc_queue)
        self.ocr_rec_thread.ocrMessage.connect(self.update_ocr)
        self.ocr_rec_thread.recMessage.connect(self.update_rec)

        self.rec_recognize_process = RecRecognizeProcess(self.rec_queue, self.rec_orc_queue)
        self.ocr_recognize_process = OcrRecognizeProcess(self.ocr_queue, self.rec_orc_queue)
    ...
    def start(self):
        ...
        self.rec_recognize_process.start()
        time.sleep(2)
        self.ocr_recognize_process.start()
        time.sleep(2)
